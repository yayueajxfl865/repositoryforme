  ...
  $.simpleCanleder = function(box, options) {
    ...
    if (r_ != "readonly") {
      box.click(function() {
        ...
        // 边界检查
        if ((boxOffset.left + box_width) > docWidth){
        	boxLeft = docWidth - box_width;
        }
        if (boxOffset.left < 0){
        	boxLeft = 0;
        }
        ...
        canlederBox.css({
          "top": boxOffset.top + box_height + 6,
          "left": boxLeft
        });
    ...
    function _buildBottom(canlederBox) {
      var $button_clear = $("<button/>").addClass("clearBtn").click(function() {
    ...
    function _getSelect(year) {
      ...
      $select = $("<select/>");
      for (var i = year - 1920; i >= 0; i--) {
        $select.append($("<option/>").text(year - i));
      }
      for (var i = 1; i <= 100; i++) { 
	    if (year + i > max_year) break;  
        $select.append($("<option/>").text(year + i));
      }
